...
hasil_uji = utils.evaluate_on_test(produk_uji, product_daily, models_produk[produk_uji])

if hasil_uji is None:
    ...
    with st.expander("Lihat data harian (aktual vs prediksi)"):
        ...
        df_uji_tampil["tanggal"] = df_uji_tampil["tanggal"].dt.strftime("%a, %d %b %Y")
        df_uji_tampil["Aktual"] = df_uji_tampil["Aktual"].round().astype(int)
        df_uji_tampil["Prediksi"] = df_uji_tampil["Prediksi"].round().astype(int)
    ...
    if baris_cek.empty:
        ...
        aktual_cek = round(float(baris_cek["Aktual"].iloc[0]))
        prediksi_cek = round(float(baris_cek["Prediksi"].iloc[0]))
        selisih_cek = aktual_cek - prediksi_cek
        cc1, cc2, cc3 = st.columns(3)
        cc1.metric("Aktual", f"{aktual_cek} cup")
        cc2.metric("Prediksi", f"{prediksi_cek} cup")
        cc3.metric("Selisih (Aktual − Prediksi)", f"{selisih_cek:+d} cup")
